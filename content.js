...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.msg == "change_color") {
        document.documentElement.style.setProperty('--main-bg-color', request.bgr);
        document.documentElement.style.setProperty('--main-text-color', request.text);
    }
});
*/
...
var container = null;
var API_Key = "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";
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.isEditable){
        if (message.action == "translate"){
            translate("Vietnamese", message.selectionText, API_Key)
            .then(text=>{
                const modifiedText = text;
                document.execCommand('insertText', false, modifiedText);
            });
        } else if (message.action == "revise"){
            revise("formal", message.selectionText, API_Key)
            .then(text=>{
                const modifiedText = text;
                document.execCommand('insertText', false, modifiedText);
            });
        } else if (message.action == "summarize"){
            summarize(message.selectionText, API_Key)
            .then(text=>{
                const modifiedText = text;
                document.execCommand('insertText', false, modifiedText);
            });
...
function createContainer(boundingRect) {
...
function createActionPopup(action) {
...
function createTextPopup(selectionText) {
...
function handleWheel(container) {
...
function translate(language, promptText, apikey) {
...
function summarize(promptText, apikey) {
    const summarizePrompt = "Summarize and shorten the following: " + promptText;
