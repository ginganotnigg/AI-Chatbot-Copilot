...
sideWindowHTML = `<div class="side_window">
    ...
    <input type="text" class="text_chat" placeholder="Type your message...">
    <button class="send_button center">Send</button>
</div>`;
...
//     <div class="chat"></div>
//     <div class="content_show">
//             <!-- Các dòng chat sẽ được thêm vào đây -->
//         </div>
//         <input type="text" class="text_chat" placeholder="Type your message...">
...
if (iconBtn != null) {
	iconBtn.addEventListener("click", () => {
		...
		const chatContent = document.querySelector(".content_show");
		const sendButton = document.querySelector(".send_button");

		// Kiểm tra nếu chiều rộng của textInput lớn hơn chiều rộng của ô chat
		if (textInput.offsetWidth > chatContent.offsetWidth) {
			// Thêm class 'multiline' để tự động xuống dòng
			textInput.classList.add("multiline");
		} else {
			// Ngược lại, loại bỏ class 'multiline' để giữ nguyên dòng
			textInput.classList.remove("multiline");
		}

		const sendButtonSideWindow = sideWindow.querySelector(
			".send_button.center"
		);
		if (sendButtonSideWindow != null) {
			sendButtonSideWindow.addEventListener("click", () => {
				sendMessage();
			});
		}

		if (textInput && sendButton) {
			// Lắng nghe sự kiện keydown trên ô nhập liệu
			textInput.addEventListener("keydown", function (event) {
				// Kiểm tra nếu phím Enter đã được nhấn
				if (event.key === "Enter") {
					// Ngăn chặn hành vi mặc định của phím Enter (chống xuống dòng)
...
// 		);
// 	}
// 	if (request.msg == "new_message") {
// 		const newMessage = request.message;
...
// 			// Kiểm tra chiều rộng của textInput và cập nhật thuộc tính CSS nếu cần
// 			updateTextInputWidth();
// 		});

// 		// Lắng nghe sự kiện click trên nút Send
...
// 			// Cập nhật chiều rộng của textInput sau khi xóa nội dung
// 			updateTextInputWidth();
// 		}
// 	}

// 	// Hàm cập nhật chiều rộng của textInput
// 	function updateTextInputWidth() {
// 		const textInput = document.querySelector(".text_chat");
// 		const chatContent = document.querySelector(".content_show");

// 		// Kiểm tra chiều rộng của textInput và chiều rộng của ô chat
// 		if (textInput.offsetWidth > chatContent.offsetWidth) {
// 			// Nếu chiều rộng của textInput lớn hơn chiều rộng của ô chat,
// 			// thêm các thuộc tính CSS để textInput tự động xuống dòng
// 			textInput.style.display = "block";
// 			textInput.style.width = "100%";
// 		} else {
// 			// Ngược lại, hiển thị textInput theo kiểu inline và giữ chiều rộng mặc định
